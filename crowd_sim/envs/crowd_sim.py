...
class CrowdSim(gym.Env):
	...
	logging.info("human number: " + str(human_num))
        ...
        #    for i in range(human_num):
        #        self.humans.append(self.generate_static_human())
        if rule == 'circle_crossing':# before it was elif
            self.humans = []
            for i in range(human_num):
                self.humans.append(self.generate_circle_crossing_human())
        elif rule == 'circle_static':
    ...
    def reset(self, number, phase='test', test_case=None):
        ...
        if number %2 == 0:#before it was elif
            self.train_val_sim = 'circle_crossing'
        ...
        if self.config.get('humans', 'policy') == 'trajnet':
            ...
            self.initial_robot_px = round(random.uniform(self.lower_center[0] + self.robot.radius + 0.5, self.big_radius - self.robot.radius - 0.5), 2) #10
            self.initial_robot_py = round(random.uniform(-self.small_radius - 2*self.robot.radius, -self.big_radius + 2*self.robot.radius), 2) #-10
            self.robot_gy = 29.0 #round(random.uniform(self.big_radius + 2*self.small_radius + self.robot.radius + 0.5, 2*self.big_radius + self.small_radius - self.robot.radius - 0.5)) #-8 #random.uniform(self.big_radius + 2*self.small_radius + self.robot.radius, 2*self.big_radius + self.small_radius - self.robot.radius)
            ...
            if len(self.w_points) >= 1:
                ...
                self.robot_direction = get_agent_direction(np.array([robot_gx, robot_gy]), np.array([self.initial_robot_px, self.initial_robot_py]))
            ...
            self.robot.set(self.initial_robot_px, self.initial_robot_py, self.w_points[0][0], self.w_points[0][1], 0, 0, np.pi / 2)
    ...
    def onestep_lookahead(self, action):
        ...
        print('Lenght of the w points: ', len(self.w_points))
	...
	timeout = 0
        ...
        #Update the way points in self.w_points
        ##print('Robot Direction {}'.format(self.robot_direction))
        #self.gif_w_points.append(self.w_points)
	...
	reward = R_danger + R_goal + R_collision + R_km + R_col_wall + R_wall_min_dist + R_stop_t + R_forward + R_way_point_dist + R_way_point_dir
        reward_values = {"Total Reward": reward,"R_dan": R_danger, "R_goal": R_goal,"R_col": R_collision, "R_km": R_km, "R_col_wall": R_col_wall, "R_wall_min_dist": R_wall_min_dist, "R_stop_t": R_stop_t, "R_forward": R_forward, "R_way_point_dist": R_way_point_dist, "R_way_point_dir": R_way_point_dir}      
        ...
        if update:
            ...
            self.robot.step(action)#, goal_x, goal_y)
